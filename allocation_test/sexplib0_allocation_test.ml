...
let stack_allocated_sexp () : Sexp.t =
  let make = Sys.opaque_identity (fun string : Sexp.t -> List [ Atom string ]) in
...
let%expect_test ("make sure a stack-allocated sexp doesn't break [maybe_globalize] and \
  ...
  =
  let measure_to_string_hum ?(here = Stdlib.Lexing.dummy_pos) sexp =
    let f () =
...
let%expect_test ("[Sexp.to_string_mach] allocates as little as possible" [@tags "no-js"]) =
  ...
  let%template[@alloc a @ m = (heap_global, stack_local)]
    ...
    let _ = (Sys.opaque_identity x : string) in
    ()
  in
